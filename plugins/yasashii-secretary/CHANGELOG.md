...
- 対応する9版（`0.8.0`、`0.9.0`、`0.9.1`、`0.9.2`、`0.10.0`、`0.10.1`、`0.10.2`、`0.12.0`、`0.13.0`）から現行版までのmigration経路を検証します。
...
- Xmindの版別既定、4象限のID・配置・色、provider選択と明示承認の境界は変わりません。

### 必要な操作

- `v0.13.0`のRelease公開を確認してから、利用中の版とscopeに合う正式なplugin更新経路を使い、新しいsessionで読み込みを確認してください。
- Claude Code向けの版別依頼文は`docs/guide/update-0.13.0.md`にあります。未公開のReleaseを推測で導入しないでください。

### 互換性上の注意

- `0.12.0`以前のrelease記録、fixture、tag、Git履歴は変更しません。source検査、版ごとの評価、公開、installed状態を別々に確認します。
...
- 「これ覚えて」と明示した後、同じ内容への了承をもう一度求められず、そのまま記録してほしい方。

### 変わること

- 明示された低リスクのmemory依頼は、その依頼自体を承認として同じturnで一度だけ保存します。
- 推測、伝聞、条件付きの内容は意味を保ったまま記録し、引用、仮定、取消、過去の照会は保存依頼として扱いません。
- 保留中の確認、訂正、同内容の再試行、checkpointだけが失敗した場合を区別し、重複記録を防ぎます。

### 設定・ファイルへの影響

- 既存のmemory、project、identity、Chatwork／Google Chat設定は自動変更しません。
- 削除、外部送信、公開、Secret、権限変更、一括処理などの確認境界は維持します。

### 必要な操作

- pluginを`0.10.2`へ更新した後、新しいsessionを開始してください。既存memoryの移行操作は不要です。

### 互換性上の注意

- `0.10.1`のidentity migrationと、それ以前のrelease記録、fixture、tag、Git履歴を維持します。

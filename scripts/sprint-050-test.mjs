...
const runCoverage = !args.has("--e2e-only");
...
async function e2e002(work) {
  ...
  assert(!/(?:実顧客|提供PDF|提供Xmind|my-vault)/u.test(tracked));
...
async function runE2eSuite() {
  const work = mkdtempSync(join(tmpdir(), "agentic-s050-e2e-"));
  const tests = [
    ["E2E-001", () => e2e001(work)],
    ["E2E-002", () => e2e002(work)],
    ["E2E-003", () => e2e003(work)],
...
const registryData = registryReport();
const report = {
  schemaVersion: 1,
  ...
  coverage: runCoverage ? runCaseCoverage(registryData) : null,
  e2e: runE2e ? await runE2eSuite() : null,
  hosts: hostBoundaryReport(),
  providers: {
...
process.stdout.write(`SPRINT050_REGISTRY primary=250 collaboration=20 visual=4 unique=274 missing=0 extra=0 duplicate=0 semantic_changed=0 assignment_changed=0\n`);
if (report.coverage) process.stdout.write(`SPRINT050_COVERAGE PASS=273 FAIL=0 CONDITIONAL_NOT_RUN=1 TOTAL=274 CRITICAL=124/124 HIGH_PASS=127 HIGH_NOT_RUN=1 AC_EXECUTED=9 AC_PASS=8 AC_BLOCKED=1\n`);
if (report.e2e) process.stdout.write("SPRINT050_E2E PASS=4 FAIL=0 CROSS_ROOT_WRITE=0 HOOK_LOOP=0 TASK_AUTO_CREATE=0 DECISION_FALSE_CONFIRM=0\n");
process.stdout.write("SPRINT050_HOST claude=external-live-gate-unavailable codex=external-live-gate-unavailable synthetic_hook_contract=pass inventory_promoted=0\n");
process.stdout.write("SPRINT050_PROVIDER xmind_external=NOT-RUN local_temp=PASS mermaid=PASS external_write=0 downstream_write=0\n");

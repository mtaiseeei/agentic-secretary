...
function registry() {
  const source = text(join(repo, "docs/spec/clarity-acceptance.md"));
...
try {
  ...
  await test("CLX-009", "Notion Taskは明示時だけ既存downstream境界へ委譲", () => {
    ...
    const routed = routeSecretaryIntent("このClarity ItemをNotionタスクにして"); assert.equal(routed.selectedSkill, "notion-tasks"); assert.equal(routed.route, "downstream-notion-task-handoff"); zeroEffect(routed);
  ...
  await test("CLX-015", "rules／serializerはboundedな結論→理由→根拠→選択を維持", () => {
    const common = text(join(repo, "plugins/secretary/rules/common-language.md")); const style = text(join(repo, "plugins/secretary/rules/styles/agentic.md")); const manifest = json(join(repo, "plugins/secretary/rules/rule-manifest.json"));
    for (const token of ["3件", "結論→理由→根拠→選択", "推定", "未検証"]) assert(common.includes(token), token);
    assert(style.includes("最終応答serializer")); assert(style.includes("結論→理由→根拠→選択")); for (const token of ["clarityOwnership", "claritySideEffectBoundary", "xmindApprovalBoundary"]) assert(manifest.forbiddenStyleOverrides.includes(token));
  });
  ...
  await test("CLX-017", "edition handoffはcommon／protectedを分離しdownstream writeを閉じる", () => {
    const handoff = json(join(repo, "adapters/downstream-clarity-handoff.json")); assert.equal(handoff.collaborationMarker, "agentic-secretary:clarity-collaboration:handoff:v1"); assert.equal(handoff.preWriteGate.status, "closed"); assert.equal(handoff.preWriteGate.writesDownstream, false);
    assert(handoff.commonPaths.includes("plugins/secretary/collaboration-inventory.json")); assert(handoff.protectedDownstreamPaths["agentic-secretary-my-vault"].some((path) => path.includes("notion-tasks"))); assert(handoff.protectedDownstreamPaths["yasashii-secretary"].includes("plugins/secretary/rules/styles/yasashii.md"));
    for (const path of handoff.commonPaths) assert.equal(/(?:my-vault|plugins\/yasashii-secretary|rules\/styles\/yasashii)/u.test(path), false, path);
  });
  ...
  await test("CLX-020", "tracked inventoryは実path／marker／digest／testと双方向一致し負例を拒否", () => {
    const inventory = loadCollaborationInventory(repo); const valid = validateCollaborationInventory(repo, inventory); assert.deepEqual(valid, { surfaceCount: 20, caseCount: 67, digestsValid: true, markersValid: true });
    ...
    const release = json(join(repo, "plugins/secretary/release-inventory.json")); const handoff = json(join(repo, "adapters/downstream-clarity-handoff.json")); assert.equal(release.collaborationInventory, "plugins/secretary/collaboration-inventory.json");
    const releaseVisual = release.xmind.visualContract; const handoffVisual = handoff.xmindContract.visual; assert.deepEqual(handoffVisual, releaseVisual); assert.deepEqual(handoff.xmindContract.editionDefaults, { "agentic-secretary": false, "agentic-secretary-my-vault": true, "yasashii-secretary": false });
    assert.deepEqual(handoff.xmindContract.providerPriorityWhenEnabled, ["xmind-mcp", "local-native-after-explicit-approval"]); assert.equal(handoff.xmindContract.writeWithoutApproval, false);
  });

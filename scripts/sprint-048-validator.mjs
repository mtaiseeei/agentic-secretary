...
check(JSON.stringify(Object.keys(hooks)) === JSON.stringify(["description", "hooks"])
  && hooks.description.includes("agentic-secretary:clarity-collaboration:hook:v1"), "Clarity Hook top-level or marker location mismatch");
...
check(text("README.md").includes("public source candidateは **0.12.0**") && text("README.md").includes("marketplace公開"), "README candidate stage mismatch");
check(text("docs/guide/getting-started.md").includes("0.12.0 source candidate") && text("docs/guide/getting-started.md").includes("installed cache")

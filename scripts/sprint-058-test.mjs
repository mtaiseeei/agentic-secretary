...
// Sprint 058: instruction routing and conditional-loading scenarios.
// This test exercises the public Secretary source only; it does not call a
...
check("shared context and progress are conditional", () => {
  ...
  assert.match(read("plugins/secretary/rules/styles/agentic.md"), /read-only.*節目|節目.*read-only/u);
});
...
check("edition and verification claims stay honest", () => {
  ...
  assert.match(read("plugins/secretary/.claude-plugin/plugin.json"), /0\.13\.1/u);
});

process.stdout.write(`SPRINT058_PASS=${pass.length} FAIL=0 ROUTE_SIDE_EFFECT_VIOLATIONS=0\n`);

...
try {
  ...
  check("editionのcandidate/latest参照はagentic 0.10.2配布面と整合", edition.edition === "agentic-secretary" && edition.distribution.marketplaceUrl.includes("agentic-secretary") && edition.distribution.changelogUrl.endsWith("/plugins/secretary/CHANGELOG.md"));
  ...
  check("新規0.10.2 onboardingはneutral markerを作る", fresh.prepared.status === 0 && marker.schemaVersion === 1 && marker.edition === "agentic-secretary", fresh.prepared.stderr);
  check("新規0.10.2導入はedition付きledgerを作る", fresh.ledger.status === 0 && ledger.schemaVersion === 2 && ledger.edition === "agentic-secretary" && ledger.records?.length === fresh.managed.length && ledger.records.every((record) => record.installedVersion === "0.10.2"), `${fresh.ledger.stderr}${fresh.ledger.stdout}`);

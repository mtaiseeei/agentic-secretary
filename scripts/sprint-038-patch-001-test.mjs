...
check("0.9.1 history remains and current release surfaces are exactly 0.11.0", () => {
  assert.equal(json(join(root, ".claude-plugin/marketplace.json")).plugins[0].version, "0.11.0");
  assert.equal(json(join(plugin, ".claude-plugin/plugin.json")).version, "0.11.0");
  assert.equal(json(join(plugin, ".codex-plugin/plugin.json")).version, "0.11.0");
...
check("Harness 0.5.1 and its public full commit are canonical", () => {
  ...
  assert.equal(harness.observedCommit, "747a8fbd06000144ca7e27330bf1d32495475fe0");
...
check("Claude Code and Codex keep distinct install routes", () => {
  ...
  assert.deepEqual(harness.hosts.claudeCode, {
    ...
    installId: "harness@agentic-harness",
    explicitEntry: "/harness",
  });
  assert.deepEqual(harness.hosts.codex, {
    marketplace: "agentic-harness-local",
    installId: "harness@agentic-harness-local",
...
check("README and build skill describe the same separate Harness release", () => {
  ...
  for (const source of [readme, build]) {
    assert(source.includes("Agentic Harness 0.5.1"));
    assert(source.includes("https://github.com/mtaiseeei/agentic-harness"));
    assert(source.includes("harness@agentic-harness"));
    assert(source.includes("harness@agentic-harness-local"));

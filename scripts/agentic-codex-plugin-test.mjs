...
check("formal Codex manifest and marketplace satisfy the shared 15-skill contract", () => {
...
check("Codex CLI ingests the local marketplace with synthetic HOME and CODEX_HOME", () => {
  ...
  try {
    ...
    assert.equal(available.available[0].version, "0.8.0");
    ...
    assert.equal(walk(cachedRoot).filter((path) => path.endsWith("/SKILL.md")).length, 15);
    ...
    assert.equal(listed.installed[0].version, "0.8.0");

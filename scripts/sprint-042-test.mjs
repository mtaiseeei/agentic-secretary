...
try {
  ...
  await test("IM-014", "cleanup applyはowned stale runtimeだけ削除", () => { const root = fixture("im014"); write(join(root, ".clarity/runtime/lock.json"), `${JSON.stringify({ owner: "agentic-secretary:clarity", expiresAt: "2026-08-01T00:00:00.000Z" })}\n`); write(join(root, ".clarity/runtime/user-note.txt"), "keep\n"); write(join(root, ".clarity/runtime/operation-live.json"), `${JSON.stringify({ owner: "agentic-secretary:clarity", expiresAt: "2026-09-01T00:00:00.000Z" })}\n`); const preview = previewRuntimeCleanup(root); assert.equal(preview.changed, false); const result = applyRuntimeCleanup(root); assert.deepEqual(result.removed, [".clarity/runtime/lock.json"]); assert(!existsSync(join(root, ".clarity/runtime/lock.json"))); assert(existsSync(join(root, ".clarity/runtime/user-note.txt"))); assert(existsSync(join(root, ".clarity/runtime/operation-live.json"))); });
...
const registryText = readFileSync(join(repo, "docs/spec/clarity-acceptance.md"), "utf8");

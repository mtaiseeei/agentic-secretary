...
check("両editionは共通契約を読み、会話copyを分離", () => {
  for (const style of ["agentic", "yasashii"]) {
    assert.match(read(`plugins/secretary/rules/styles/${style}.md`), /conversation-contract\.md/u);
    const copy = JSON.parse(read(`plugins/secretary/rules/copy/${style}.json`));
    assert.ok(copy.surfaces.conversation.voice, `${style} voice copy`);
  }
});

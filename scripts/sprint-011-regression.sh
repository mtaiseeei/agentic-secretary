...
serializer_contract_ok(){
  ...
  grep -q '最終応答serializer（通常報告の唯一の正本）' "$file" &&
    grep -Eq '必要なtool callを終えた後にserializerを1回だけ適用|短い操作は最終応答へまとめます' "$file" &&
...
check "共通表現とyasashii styleは同一turn read-onlyで途中出力しない" \
  "grep -q '短いRead、routing、read-only確認は途中メッセージを出さず' '$COMMON_RULES' && grep -q '長いread-only作業では節目の進捗' '$RULES'"
...
cp "$RULES" "$WORK/bad-owner.md"; perl -pi -e 's/必要なtool callを終えた後にserializerを1回だけ適用/serializerを適用/g; s/短い操作は最終応答へまとめます/操作は最終応答へまとめます/g' "$WORK/bad-owner.md"

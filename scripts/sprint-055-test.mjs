...
const work = mkdtempSync(join(tmpdir(), "agentic-s055-"));
...
function event(root, type, itemId, payload) { return appendEvent(root, { type, itemId, actor: "sprint-055-test", payload }); }
...
process.stdout.write(`SPRINT055_PASS=${results.length - failed.length} FAIL=${failed.length} TOTAL=${results.length}\n`);

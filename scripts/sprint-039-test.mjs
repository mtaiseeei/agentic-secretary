...
function makeWorkspace(name, { edition = "agentic-secretary", secretaryId = randomUUID(), displayName = "Alex" } = {}) {
  ...
  execFileSync("git", ["config", "user.name", "Fixture User"], { cwd: workspace, stdio: "ignore" });
  execFileSync("git", ["config", "user.email", "fixture@example.invalid"], { cwd: workspace, stdio: "ignore" });
...
try {
  ...
  throwsNoMutation("registryは確認前write 0", registryHome, () => registerWorkspace({ home: registryHome, workspace: canonical.workspace, edition: "agentic-secretary" }), /明示確認前/u);
  check("registryは最小metadataだけを保存", () => { registerWorkspace({ home: registryHome, workspace: canonical.workspace, edition: "agentic-secretary", confirm: true }); const registry = readRegistry(registryHome); assert.equal(registryContainsOnlyAllowedMetadata(registry), true); const text = JSON.stringify(registry); for (const forbidden of ["memory_body", "conversation_body", "customer_name", "api_token", "Alex"]) assert.doesNotMatch(text, new RegExp(forbidden, "iu")); });
  const foreign = join(sandbox, "foreign-repo"); mkdirSync(foreign); const foreignBefore = treeSnapshot(foreign);
  check("別repo cwdからcanonical workspaceを副作用0で解決", () => { const result = resolveCanonicalWorkspace({ home: registryHome, edition: "agentic-secretary" }); assert.equal(result.status, "resolved"); assert.equal(result.workspace, realpathSync(canonical.workspace)); assert.deepEqual(treeSnapshot(foreign), foreignBefore); assert.equal(existsSync(join(foreign, "secretary")), false); });
  check("registry再登録は差分0", () => { const before = treeSnapshot(registryHome); assert.equal(registerWorkspace({ home: registryHome, workspace: canonical.workspace, edition: "agentic-secretary", confirm: true }).status, "unchanged"); assert.deepEqual(treeSnapshot(registryHome), before); });
  ...
  const opposite = makeWorkspace("opposite", { edition: "yasashii-secretary" }); check("反対edition登録を拒否", () => assert.throws(() => registerWorkspace({ home: makeHome("opposite-home"), workspace: opposite.workspace, edition: "agentic-secretary", confirm: true }), /opposite-edition/u));
  const symlinkWorkspace = join(sandbox, "canonical-link"); symlinkSync(canonical.workspace, symlinkWorkspace); check("workspace symlinkを安全停止", () => assert.throws(() => registerWorkspace({ home: makeHome("link-home"), workspace: symlinkWorkspace, edition: "agentic-secretary", confirm: true }), /unsafe-workspace/u));
  const moved = join(sandbox, "canonical-moved"); renameSync(canonical.workspace, moved); check("移動前registryはmissingを返す", () => assert.equal(resolveCanonicalWorkspace({ home: registryHome }).status, "missing")); check("同じstable IDの移動先を再登録", () => { assert.equal(registerWorkspace({ home: registryHome, workspace: moved, edition: "agentic-secretary", confirm: true }).status, "moved"); assert.equal(resolveCanonicalWorkspace({ home: registryHome }).workspace, realpathSync(moved)); });
  ...
  check("下流handoffはSHA・共通・除外・保護・rollbackを宣言", () => { const report = JSON.parse(execFileSync(process.execPath, [join(ROOT, "scripts/sprint-039-handoff.mjs"), "--root", ROOT], { encoding: "utf8" })); if (report.candidateGitStatus === "clean") assert.match(report.agenticFullSha, /^[0-9a-f]{40}$/u); else assert.equal(report.agenticFullSha, null); assert.ok(["clean", "dirty", "git-free"].includes(report.candidateGitStatus)); assert.match(report.commonTreeSha256, /^[0-9a-f]{64}$/u); assert.ok(report.commonPaths.includes("plugins/secretary/skills/name/SKILL.md")); assert.ok(report.excludedPaths.includes("docs/**")); assert.ok(report.protectedDownstreamPaths["agentic-secretary-my-vault"].includes("AGENTS.md")); assert.ok(report.rollback); });
  ...
  check("report-schemaはname／clarity Skillを含む正式22面を受理", () => { const output = execFileSync("python3", [join(ROOT, "scripts/check-report-schema.py"), "--plugin-root", join(ROOT, "plugins/secretary")], { encoding: "utf8" }); assert.match(output, /surfaces=22/u); });

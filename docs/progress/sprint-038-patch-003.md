...
- 製品・回帰candidate commit: `18a41825b5b28d9c8519fab94360619b8a35e87a`
- candidate tree: `76d2321cc67afab1347c86dc449dd52207ae39fc`
...
- `plugins/secretary/scripts/lib/conversation-migration.mjs`
  ...
  - temp名へprocess IDとrandom nonceを含める。開始前から存在する旧形式の
    `.<basename>.conversation-migration-<pid>`を上書き・unlinkしない。
...
- `scripts/sprint-038-patch-003-conversation-migration-test.mjs`
  ...
  - 開始前からある旧temp名collision canaryの上書き・unlink 0件。
  - dry-run、marker片側、旧section複数、ownership不明、stale plan。
  - rename前failure時のtarget mtime不変、owned temp cleanup、失敗後retry。
  - rename後failure時のatomic相当rollback temp、元bytes復元、失敗後retry、成功後rerun。
  - 明示rollbackと開始前rollback collision canary。
  - Windows nativeではdrive letter、backslash、空白、日本語の実absolute pathを必須にする`--require-windows`。
...
開始HEADの`git archive`へcandidate 5 filesを重ねた一時copyで実行した。
...
必須観測は、40桁candidate SHA、OS／Node、exit、9 caseのPASS／FAIL、drive letter／backslash／空白／日本語target、

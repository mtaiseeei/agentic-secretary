...
件数は診断補助で確認できる。一方、「状態以外の情報で読みにくい」と
「PJ固有のガードレールが必要」は、LLMが実内容を読んだ理由を短く示して判断できる。
`promotion-status` は任意のread-only診断である。昇格の書込みは、上の理由と利用者の承認を
既存 `promote-full` の入力として明示し、同コマンドの状態・保護・rollback検査を通してだけ行う。
...
- journalとdecisionsを日付キーで読み、逆時系列のMarkdownに整形する任意のread-only集計手段である。
- 日付範囲とtypeを組み合わせられる。`--grep` は日付だけでは答えられない横断検索を担う。
- 同一入力・同一固定時刻ではbyte単位で同一出力になることを目標とする。
- 必要な原本が安全に取得済みなら、LLMはこの集計を追加実行せず、利用者の問いに合わせて原本から応答を組み立ててよい。期間・種類・出典・変更履歴と、decision正本をjournal `decided` より優先する重複抑止は同じである。
- weekly相当の整理はjournal原本の `did` / `decided` / `next` を分け、activeとarchiveの対象範囲、原本path、日付を追えるようにする。
...
- 一人称: 私（既定） | 利用者が明示した、前後空白除去後1〜16 Unicode code pointの改行なし文字列
...
既定値は、口調=丁寧（標準）、一人称=私、専門用語=ふつう、報告=みじかく（内容依存）、決定確認=都度。一人称の欠損は「私」に戻し、口調・秘書名・役割から推測しない。一人称は既存の `oneLine` とsecret検査に加えて上記長さだけを検証し、意味parserや固定allowlistは持たない。応答時にLLMが一人称としての意味を判断し、名前・実行状態・安全ruleと競合する場合は「私」または自然な主語省略を使うが、保存値を黙って変更しない。

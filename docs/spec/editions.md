...
- Windows／macOS／Linuxのproject・memory・TODO・settings・文書保存、path guard、journal、rollbackの意味契約
...
Sprint 038の `0.9.0` 公開後、Harness互換参照だけを更新したpatch version `0.9.1` は当時のcandidateとして記録する。
...
`0.9.1` のAgentic／Yasashii公開後、Windowsネイティブの記録・保存互換を直す後方互換patchは
`0.9.2` を現在candidateとする。Agentic共通coreを先に独立評価し、PASSした完全SHAからだけ

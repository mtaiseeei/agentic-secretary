...
- 主眼: development-pointerの古いworkspace snapshotだけで現在判断せず、利用可能な正本repoをread-onlyに確認する。同時に、Clarityの指定入口だけが内部opt-inでancestor symlink aliasを物理rootへ固定し、従来の内部symlink／Git／write境界を弱めない。
...
正当な配置がある。Clarityの指定入口は専用root resolverで内部opt-inし、利用者へflag／設定を求めずancestor aliasを物理rootへ固定して扱う。
...
- workspace ancestorだけがsymlinkのRepoは、Clarity指定入口の内部opt-inでphysical pathと同じRepo identity・Clarity結果へ到達する。
...
### B. Clarity限定ancestor symlink内部opt-in

1. root policyは`allowAncestorSymlinks: false`相当を既定とする。一般`workingRoot(value)`はoption省略／falseの両方で、project／memory／update等の既存入口と同じ既定拒否を維持する。
2. trueはClarity専用root resolverだけがrequest中に内部指定する。利用者向けCLI flag／設定は増やさず、共通filesystemや他Skillへ緩和を伝播させない。結果は適用policyがClarity internal opt-inであることを識別できる情報を返す。
...
1. 内部opt-inを使う指定入口は、Clarity CLIのRepo root指定、Clarity core公開操作のRepo root指定、link prepare／accept／finalize等のlocal Repo root指定、projectionのRepo root指定、DriftのRepo root指定、Secretary adapterの`canonicalRepo` local root／Secretary-local Clarity root指定、Clarity Hookのcwd／Repo root discoveryである。全て同じroot policyとphysical containmentを使う。DriftのDecision／implementation source locatorは対象外である。
2. 一般safe filesystemのerrorであるancestor alias未許可と、Clarity入口のroot自身symlink、root内unsafe symlink、broken／directory以外のancestor、alias／physical root changedを区別する。
3. errorは何が起きたか、`changed:false`、次に指定できる安全なpathを示し、利用者へopt-in操作や内部例外だけを返さない。
...
5. 一般`workingRoot`はoption省略／falseでancestor symlinkを従来どおり`working-root-unsafe`として拒否する。対応するClarity指定入口だけが内部で`allowAncestorSymlinks: true`を使い、利用者操作なしでworkspace ancestor aliasから次のClarity判定へ到達する。
...
13. CLI／core／link／projection／Drift／Secretary adapter／Hookのentrypoint matrixが同じClarity内部root policyを使い、適用policyの由来を返し、ancestor可・root自身・内部unsafe・root changedのerrorを区別する。
...
- 一般`workingRoot`＋workspace ancestor alias、option省略／falseと、同じrootに対するClarity指定入口の内部opt-in。
- Clarity指定入口が内部opt-inするがworking root自身がsymlink。
...
- CF-001またはAR-014には、development-pointerの`canonicalRepo`記載path自体がworkspace ancestor alias配下にある組合せfixtureを含める。
- AR-001〜014はalias/workspace/repoのworkspaceだけをsymlinkにし、一般option省略／false、Clarity内部opt-in、alias／physical、未初期化／初期化済み、preview／applyを組み合わせる。
- AC7のapplyはsynthetic fixtureだけで行い、宣言済み`.clarity/**`の前後diffと外部canaryを取る。実顧客repoでは実行しない。
- CLI、core、link、projection、Drift、Secretary adapter、Hookの各Repo root指定入口を直接実行し、単に共有helperのunit testだけで全入口PASSとしない。AR-011はread-onlyのDrift source locator negativeに限定し、resolve／apply／Git commitを実行しない。
